...
func Open() (*sql.DB, error) {
	db, err := sql.Open("mysql", "root:dev-root@localhost:5003/ProyectoFinalDra")
	if err != nil {
		return db, err
	}
...
func InitDB() error {
	//Create tables
	createTestingData := `create table users(
	...
	if err := executeFastQuery(createTestingData); err != nil {
		return err
	}
...
func executeFastQuery(query string) error {
	db, err := Open()
	if err != nil {
		return err
	}
